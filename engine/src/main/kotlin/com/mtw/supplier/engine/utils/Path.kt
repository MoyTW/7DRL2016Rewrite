...
class LinePathBuilder(val targetPos: XYCoordinates, val seededRand: SeededRand? = null, val spread: Int = 0): PathBuilder {
    override fun build(startPos: XYCoordinates): Path {
        if (spread > 0 && seededRand == null) {
            throw IllegalArgumentException("You can't have spread without a rand")
        }
